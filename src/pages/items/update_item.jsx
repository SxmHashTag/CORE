...
const UpdateItem = () => {
  const validationSchema = Yup.object({
    ...
    case: Yup.string().required("Case ID is required"),
  ...
  const locationChoices = ["Inbeslag", "Digi", "Rst", "See journal", "other"];
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { resetForm }) => {
      if (values.book_date) {
        values.book_date = values.book_date.format("YYYY-MM-DD");
      }
      if (values.acquisition_date) {
        values.acquisition_date = values.acquisition_date.format("YYYY-MM-DD");
      }
      ...
      axiosInstance
        ...
        .catch((err) => {
          setMessage(
            <MyMessage
              messageText="Failed to update item!"
              messageColor="#f8d7da"
            />
          );
          console.error(err);
    ...
    axiosInstance
      ...
      .then((res) => {
        ...
        formik.setValues({
          ...
          imei: data.imei || "",
          owner: data.owner || "",
          book_date: data.book_date ? dayjs(data.book_date) : null,
        ...
        <Box className="FormBox" sx={{ p: 3 }}>
          ...
          >
            ...
            <SelectForm
              ...
              options={locationChoices.map((location) => ({
                value: location,
                label: location,
              }))}
            />
            ...
            <LocalizationProvider dateAdapter={AdapterDayjs}>
              <DatePicker
                label="Book Date"
                value={
                  formik.values.book_date
                    ? dayjs(formik.values.book_date)
                    : null
                }
                onChange={(newValue) =>
